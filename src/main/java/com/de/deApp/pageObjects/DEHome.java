...
public class DEHome extends Page {
	...
	public UIElement getLnk_Settings() {
		return new UIElement(loc_lnk_Settings, getPageName(), "lnk_Settings");
	}
	...
	public DEHome goToManageItems() {
		new Actions(DriverManager.getDriver()).moveToElement(getLnk_ManageItems()).perform();
		return new DEHome();
	}
	...
	public void clickOnOption(String option) {
		try {
			...
			DriverManager.getDriver().get("http://newdiningedge.baitek.org:8100/manage-items");
		} catch (Exception e) {
			e.printStackTrace();
			DriverManager.getDriver().navigate().to("http://newdiningedge.baitek.org:8100/manage-items");
		}
	}
